...
def max_resource_and_margin(inventory: np.ndarray) -> tuple[int, int]:
    ...
    margin = int(inventory[sorted_resources[-1]]) - int(inventory[sorted_resources[-2]])
...
class AlternatingSpecialist(puppeteer.Puppeteer[int]):
    ...
    def __init__(
        self, *, targets: Sequence[Resource], interactions_per_target: int, margin: int
    ) -> None:
        """Initializes the puppeteer.
    ...
    def step(
        ...
        timestep = collect_or_interact_puppet_timestep(timestep, target, self._margin)
...
class RespondToPrevious(puppeteer.Puppeteer[Resource]):
    ...
    def step(
        ...
        timestep = collect_or_interact_puppet_timestep(timestep, response, self._margin)
