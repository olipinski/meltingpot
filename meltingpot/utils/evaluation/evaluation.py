...
def run_and_observe_episodes(
    ...
    with contextlib.ExitStack() as stack:
        ...
        subscribe(focal_observables.names, on_next=data["focal_player_names"].append)

        background_return_subject = ReturnSubject()
        subscribe(background_observables.timestep, background_return_subject)
        subscribe(
            background_return_subject, on_next=data["background_player_returns"].append
        )
        subscribe(
        ...
        subscribe(
            background_observables.names, on_next=data["background_player_names"].append
        )
