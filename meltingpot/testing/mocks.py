...
def _build_mock_substrate(
    *,
    ...
    mock_substrate.reward_spec.return_value = (timestep_spec.reward,) * num_players
...
def build_mock_substrate(
    *,
    num_players: int = 8,
    num_actions: int = 8,
    observation_spec: Mapping[str, dm_env.specs.Array] = SUBSTRATE_OBSERVATION_SPEC,
) -> ...:
    """Returns a mock Substrate for use in testing.
...
def build_mock_scenario(
    *,
    num_players: int = 8,
    num_actions: int = 8,
    observation_spec: Mapping[str, dm_env.specs.Array] = SCENARIO_OBSERVATION_SPEC,
) -> ...:
