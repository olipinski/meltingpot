...
function Progress:start()
  ...
  if self.votingValues.type == 'deliberation' then
    ...
    self.inVotingRound_tensor:val{0}
  end
...
function Progress:setVotingRoundStatus(voting)
  ...
  if voting == true then
    self.inVotingRound_tensor:val{1}
...
function Progress:checkImpostorVoteWin()
  -- Check whether the Impostors won by tagging and end the episode.
  if (self:numActiveCrewmates() <= self.crewmatesLeftForImpostorWin) then
    -- Add to events as a win condition.
    events:add('win', 'dict',
               'condition', 'impostor vote win') -- string
    self:gameEnd(self.crewmate_vote_reward, self.impostor_vote_reward)
    return true
  end
  return false
end
