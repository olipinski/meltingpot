...
def verbose_fn(env_timestep, player_index, current_player_index):
    ...
    if (
        env_timestep.observation["WORLD.RACE_START"].any()
        and player_index == current_player_index
    ):
...
def main():
    ...
    parser.add_argument(
        "--level_name",
        type=str,
        default="boat_race__eight_races",
    ...
    with config_dict.ConfigDict(env_config).unlocked() as env_config:
        ...
        game_object_utils.get_first_named_component(
            env_config.lab2d_settings["simulation"]["prefabs"]["seat_L"], "BoatManager"
