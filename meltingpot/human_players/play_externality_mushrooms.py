...
def verbose_fn(env_timestep, player_index, current_player_index):
    ...
    destroyed_hihe = env_timestep.observation[f"{lua_index}.DESTROYED_MUSHROOM_HIHE"]
    destroyed_fize = env_timestep.observation[f"{lua_index}.DESTROYED_MUSHROOM_FIZE"]
    destroyed_zife = env_timestep.observation[f"{lua_index}.DESTROYED_MUSHROOM_ZIFE"]
...
def main():
    ...
    parser.add_argument(
        "--level_name",
        type=str,
        default="externality_mushrooms__dense",
