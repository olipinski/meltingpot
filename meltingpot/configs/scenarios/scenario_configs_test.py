...
class ScenarioConfigTest(parameterized.TestCase):
    ...
    def test_no_missing_role_assigments(self, scenario):
        background_roles = set(
            role for n, role in enumerate(scenario.roles) if not scenario.is_focal[n]
        )
        supported_roles = {role for role, bots in scenario.bots_by_role.items() if bots}
        unsupported_roles = background_roles - supported_roles
        self.assertEmpty(
            unsupported_roles,
            f"Background roles {unsupported_roles!r} have not been " f"assigned bots.",
        )
